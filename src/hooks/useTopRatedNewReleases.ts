...
type ReleaseType = 'combined' | 'movie' | 'tv' | 'anime';
...
const useTopRatedNewReleases = (
  type: ReleaseType = 'combined'
): TopRatedNewReleases => {
  ...
  const {
    ...
    type === 'tv' || type === 'anime' ? undefined : movieEndpoint
  );
  ...
  const {
    ...
    type === 'movie' || type === 'anime' ? undefined : tvEndpoint
  );
